...
export const Reviews = ({productId,averageRating}) => {
  ...
  return (
        <Stack rowGap={5} alignSelf={"flex-start"}  width={is480?"90vw":is840?"25rem":'40rem'}>
            ...
            {/* reviews mapping */}
            <Stack rowGap={2} >
                {reviews && reviews?.map((review)=>(<ReviewItem key={review?._id} id={review?._id} userid={review?.user?._id} comment={review?.comment} createdAt={review?.createdAt} rating={review?.rating} username={review?.user?.name} />))}
            </Stack>
